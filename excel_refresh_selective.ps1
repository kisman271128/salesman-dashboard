...
# FIXED: Only works with DbaseSalesmanWebApp.xlsm

param(
    [string]$ExcelFile = "C:\Dashboard\DbaseSalesmanWebApp.xlsm",  # FIXED: Default to .xlsx
...
Write-Log "=== PARAMETER VALIDATION - XLSM ONLY ==="
...
if ([string]::IsNullOrEmpty($ExcelFile) -or -not (Test-Path $ExcelFile)) {
    Write-Log "Parameter file not found or empty, attempting auto-detection for .xlsm..."
    
    # Get current directory
    $CurrentDir = Get-Location
    Write-Log "Current directory: $CurrentDir"
    
    # FIXED: Only check for .xlsm file
    $TestFileXlsx = Join-Path $CurrentDir "DbaseSalesmanWebApp.xlsm"
    
    Write-Log "Testing for: $TestFileXlsx"
    
    if (Test-Path $TestFileXlsx) {
        $ExcelFile = $TestFileXlsx
        Write-Log "AUTO-DETECTED: $ExcelFile (.xlsm)"
    } else {
        Write-ErrorLog "Excel file not found: $TestFileXlsx"
        Write-ErrorLog "Required file: DbaseSalesmanWebApp.xlsm"
        ...
        if ($ExcelFiles) {
            ...
            Write-ErrorLog "Only DbaseSalesmanWebApp.xlsm is supported by this script"
        } else {
...
if (Test-Path $ExcelFile) {
    ...
    if ($FileInfo.Name -ne "DbaseSalesmanWebApp.xlsm") {
        Write-ErrorLog "Incorrect file detected: $($FileInfo.Name)"
        Write-ErrorLog "This script only works with: DbaseSalesmanWebApp.xlsm"
        exit 1
    }
...
function Kill-ExcelProcesses {
    Write-Log "AUTO: Terminating all Excel processes for automation..."
    try {
...
function Refresh-Excel-Selective {
    ...
    try {
        ...
        if ($FinalFileInfo.LastWriteTime -gt $RecentTime) {
            ...
            $SuccessMessage = @"
Excel Refresh Successful - XLSM Only
====================================
Time: $(Get-Date)
...
Method: Selective file handling (DbaseSalesmanWebApp.xlsm only)
...
if ($FileInfo.Name -ne "DbaseSalesmanWebApp.xlsm") {
    ...
    Write-ErrorLog "This script only supports: DbaseSalesmanWebApp.xlsm"
    exit 1
}

Write-Log "Excel file confirmed: DbaseSalesmanWebApp.xlsm"
...
if ($Success) {
    ...
    $FailureMessage = @"
Excel Refresh Failed - XLSM Only
================================
Time: $(Get-Date)
File: $ExcelFile
Attempts: $MaxRetries
Duration: $([math]::Round($TotalTime, 1)) seconds
Error: Could not access or refresh DbaseSalesmanWebApp.xlsm

Troubleshooting:
1. Check if DbaseSalesmanWebApp.xlsm is open in Excel manually
2. Verify file permissions
3. Check data connections manually
4. Try running script as administrator
5. Contact IT support if issue persists

Required File: DbaseSalesmanWebApp.xlsm (only)
Log File: $LogFile
"@
